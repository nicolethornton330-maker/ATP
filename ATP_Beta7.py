...
class EmployeesFrame(ttk.Frame):
    ...
    def _add_employee(self):
        ...
        ttk.Button(win, text="Save", command=save_changes).pack(side="left", padx=4)
        ttk.Button(btn_frame, text="Delete Employee", command=delete_employee).pack(side="left", padx=4)
        ttk.Button(win, text="Cancel", command=win.destroy).pack(side="left", padx=4)
...
class ReportsFrame(ttk.Frame):
    ...
    def auto_expire_points(self):
        """Automatically apply -1.0 rolloff deductions and generate an HRIS-ready CSV audit."""
        ...
        for rec in expired_recs:
            ...
            current_total = float(rec["point_total"] or 0)
            new_total = max(current_total - 1.0, 0.0)  # Prevent negative totals

            # Insert rolloff deduction record into history
            self.conn.execute("""
                INSERT INTO points_history (employee_id, point_date, points, reason, note, flag_code)
                VALUES (?, ?, ?, ?, ?, ?);
            """, (
                ...
                "Automatic 2-Month Point Expiration",
                "AUTO"
            ))

            # Update employee totals and next rolloff/perfect attendance dates (leave last_point_date unchanged)
            rolloff_date, perfect_date = calc_rolloff_and_perfect(date.today())
            self.conn.execute("""
                UPDATE employees
                   SET point_total=?,
                       rolloff_date=?,
                       perfect_attendance=?
                 WHERE employee_id=?;
            """, (new_total, rolloff_date.isoformat(), perfect_date.isoformat(), emp_id))

            expired_count += 1

            # Add HRIS-aligned row for CSV log
            log_rows.append([
                emp_id,
                last_name,
                first_name,
                ymd_to_us(today),    # Rolloff Date in MM-DD-YYYY
                "-1.0",              # Point (deduction)
                "2 Month Roll Off",  # Reason
                "",                  # Empty Note column
                f"{new_total:.1f}"   # Updated total
            ])
        ...
        if expired_count > 0:
            ...
            messagebox.showinfo(
                "Rolloff Applied",
                f"Applied -1.0 point adjustment to {expired_count} employee(s).\n"
                f"Audit saved as {os.path.basename(path)}"
            )
            self.app.set_status(f"Auto rolloff complete — {expired_count} employee(s) adjusted.", ok=True)
        else:
