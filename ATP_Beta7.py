...
class EmployeesFrame(ttk.Frame):
    ...
    def _on_tree_double_click(self, event):
        ...
        win.transient(self)
        win.grab_set()
        win.geometry("420x460")  # slightly taller for breathing room

        # --- Main form area ---
        form_frame = ttk.Frame(win, padding=20)
        form_frame.pack(fill="both", expand=True, pady=(0, 10))

        ttk.Label(form_frame, text=f"Editing Employee #{emp_id}",
        ...
        for label, key in fields:
            ttk.Label(form_frame, text=label).pack(anchor="w", pady=(4,0))
            val = ymd_to_us(rec[key]) if rec[key] else ""
            vars[key] = tk.StringVar(value=val)
            ttk.Entry(form_frame, textvariable=vars[key], width=36)\
            ...
            self.conn.execute("""
                UPDATE employees
                   SET last_name=?,
                       ...
                       point_warning_date=?
                 WHERE employee_id=?;
            """, (updates["last_name"], updates["first_name"],
                  updates["last_point_date"], updates["rolloff_date"],
                  updates["perfect_attendance"],
    ...
    def _open_inline_edit(self, emp_id, item, *_):
        ...
        win.geometry("420x500")  # taller to ensure space for button
        ...
        for label, key in fields:
            ...
            val = ymd_to_us(rec[key]) if rec[key] else ""
            vars[key] = tk.StringVar(value=val)
            ttk.Entry(form_frame, textvariable=vars[key], width=36).pack(anchor="w", pady=(0,6))
        ...
        btn_frame.grid(row=1, column=0, sticky="ew", pady=(12, 8))
        ...
        save_btn = tk.Button(btn_frame,
                             ...
                             pady=8,
                             cursor="hand2")
        save_btn.grid(row=0, column=0, padx=20, sticky="ew")
