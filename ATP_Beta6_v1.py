...
# define the database file path first
DB_FILE = "attendance_MASTER.db"
...
#if not os.path.exists("attendance_MASTER.db"):
...
class App(tk.Tk):
    def __init__(self):
        ...
        # --------------------------------------------
        # Database setup (safe check inside __init__)
        # --------------------------------------------
        if not os.path.exists("attendance_MASTER.db"):
            import sqlite3
            dbconn = sqlite3.connect("attendance_MASTER.db")
            c = conn.cursor()
            c.executescript(SCHEMA_BASE)
            conn.commit()
            conn.close()
            print("✅ New attendance_MASTER.db created because none was found.")
        else:
            print("ℹ️  Existing attendance_MASTER.db detected — skipping initialization.")

        # Establish connection for the rest of the app
        self.c = conn()
...
class EmployeesFrame(ttk.Frame):
    ...
    def refresh(self):
        for r in self.tree.get_children():
            self.tree.delete(r)
        cur = self.c.execute("""
            SELECT e.employee_id, e.last_name, e.first_name, e.last_point_date, e.perfect_attendance, e.rolloff_date,
                   COALESCE(ROUND(SUM(p.value),2),0.0) AS total_points, e.point_warning_date
              FROM employees e
              LEFT JOIN points p ON p.employee_id = e.employee_id
          GROUP BY e.employee_id, e.last_name, e.first_name, e.last_point_date, e.perfect_attendance, e.rolloff_date, e.point_warning_date
          ORDER BY e.last_name, e.first_name;
        """)
        rows = cur.fetchall()
        q = (self.search_var.get() or "").strip().lower()
...
class PointsFrame(ttk.Frame):
    ...
    def refresh(self):
        ...
        cur = self.c.execute("""
            SELECT p.id AS point_id, p.employee_id,
               e.last_name || ', ' || e.first_name AS employee_name,
               p.date, p.value, p.reason
            FROM points p
        ...
        for r in cur.fetchall():
            # Add two empty placeholders so the tuple has 8 items
            rows.append(r + ("", ""))
    ...
    def fetch_summary(self):
        return self.c.execute("""
            SELECT e.employee_id, e.last_name, e.first_name,
                   ...
                   e.last_point_date, e.perfect_attendance, e.rolloff_date
              FROM employees e LEFT JOIN points p ON p.employee_id = e.employee_id
          GROUP BY e.employee_id, e.last_name, e.first_name, e.last_point_date, e.perfect_attendance, e.rolloff_date
          ORDER BY e.last_name, e.first_name;
        """).fetchall()
...
def main():
    if not os.path.exists("attendance_MASTER.db"):
        import sqlite3
        dbconn = sqlite3.connect("attendance_MASTER.db")   # renamed variable
        c = dbconn.cursor()
        c.executescript(SCHEMA_BASE)
        dbconn.commit()
        dbconn.close()
